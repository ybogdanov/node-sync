...
Function.prototype.future = function(obj /* arguments */) {
    ...
    args.push(function syncCallback(err) {
        ...
        var err = cb_args.shift();
        if (err) {
            error = err instanceof Error ? err : new Error(err); // todo: cast to Error or not cast to Error?
            return;
        }
    ...
    Future.__defineGetter__('value', Future);
    Future.__defineGetter__('error', function(){
        try {
            Future();
        }
        catch (e) {
