...
Function.prototype.future = function(obj /* arguments */) {
    ...
    var args = Array.prototype.slice.call(arguments),
        ...
        obj = args.shift(),
        fiber = Fiber.current,
        future = new SyncFuture(fiber);

    // virtual callback, push it as last argument
    args.push(function syncCallback(err) {
        // forbid to call twice
        if (future.called) return;
        future.called = true;
        ...
        var err = args.shift();
        if (err) {
            future._error = err instanceof Error ? err : new Error(err); // todo: cast to Error or not cast to Error?
        }
        else {
            // result pieces as other arguments (if one, shrink it)
            var result = args;
        ...
        if (future.yielding && Fiber.current !== fiber) {
            future.yielding = false;
            // catch 'This Fiber is already running' error
            try {
                fiber.run();
            }
